...
export default function WorkflowCard({ workflow }: { workflow: Workflow }) {
    ...
    return (
        <Card className="border border-separate shadow-xs rounded-lg overflow-hidden hover:shadow-sm dark:shadow-primary/30">
            <CardContent className="p-4 flex items-center justify-between h-[50px]">
                <div className="flex items-center justify-end space-x-3">
                    ...
                    <div>
                        <h3 className="text-base font-bold text-muted-foreground flex items-center">
                            ...
                            {isDraft && (<span className="ml-2 px-2 py-0.5 text-xs font-medium bg-yellow-100 text-yellow-700 rounded-full">Draft</span>)}
                        </h3>
                    </div>
                </div>
                <div className="flex items-center space-x-2">
                    <Link href={`/workflow/editor/${workflow.id}`} className={cn(buttonVariants({ variant: "outline", size: "sm" }), "flex items-center gap-2")}><ShuffleIcon size={16} />Edit</Link>
